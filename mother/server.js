...
const deleteCategoryTree = (rootId) => {
    ...
    db.prepare(`UPDATE templates SET category_id = NULL WHERE category_id IN (${ph})`).run(...ids);
    db.prepare(`UPDATE maintenance_items SET category_id = NULL WHERE category_id IN (${ph})`).run(...ids);
